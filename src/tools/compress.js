...
const fs = (typeof window !== 'undefined' && window.megspotAPI && window.megspotAPI.fs) ? window.megspotAPI.fs : null
...
export class SnapshotHelper {
  ...
  async load(path) {
    ...
    try {
      if (fs && !fs.existsSync(TEMP_PATH)) {
        fs.mkdir(TEMP_PATH, { recursive: true })
      }
  ...
  // 验证文件是否为MegSpot工程文件
  async validate(path) {
    const data = await (fs ? fs.readFile(path) : Promise.resolve(null))
  ...
  async decompress(snapPath, files, encoding = 'utf8') {
    try {
      ...
      for (const key of keys) {
        ...
        if (file.dir) {
          if (fs && !fs.existsSync(file.name)) fs.mkdir(file.name, { recursive: true })
      ...
      console.log('decompress success! path:', path.resolve(TEMP_PATH))
      return res
    } catch (e) {
      console.error(e)
      return false
